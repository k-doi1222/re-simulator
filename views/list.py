...
# 目標到達価格は目安なのでその後ろ。
# 目標に乗せる価格は「△150 いくらまで下げるか」と「そのときの指値率」の2つで見る。
# ○200 は列をやめた（2026-09-20 ユーザー判断。詳細画面では今までどおり出る）。
view["150指値率"] = (1 - view["到達150"] / view["販売価格"]) * 100

COLS = ["状況", "登録日付", "物件名", "所在地", "cf基準",
        "築年数", "価格", "満室利回り", "到達150", "150指値率",
...
with st.container(key="fulltable"):
    event = st.dataframe(
        ...
        column_config={
            ...
            "150指値率":  percent1("△150 指値率",
                                   help="販売価格から △150 の価格まで、何%下げることになるか"),
            "築年数":     count("築年数", " 年"),
